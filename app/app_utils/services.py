...
def get_memory_service():
    """Process-wide memory service: Vertex AI Memory Bank when deployed.

    The traveller's preferences, allergies, and past trips live here. Without
    this registered, the agent's `load_memory` / `preload_memory` tools read from
    an empty bank and every conversation starts from scratch — which is what
    happened before, because the memory service was constructed in agent.py and
    then never passed to a Runner.
    """
